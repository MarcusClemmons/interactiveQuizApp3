...
function handleSubmit() {
    ...
    const auth = getAuth();
    const userEmail = auth.currentUser ? auth.currentUser.email : null; // Get user's email
    const responsesCollection = collection(db, "responses");
    if (!userEmail) {
        console.error("User email is not available.");
        return; // Exit the function if email is not available
    }
    ...
    .then(() => {
        console.log("Responses saved to Firestore");
